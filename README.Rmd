...
Data can be pulled from the SET database using load_set_data() for SET data or load_mh_data() for SET and MH data, respectively. Use the park or network code options to limit the data to a specific park(s) or I&M network(s).
...
The function calc_change_cumu() can be used to calculate station-level cumulative surface elevation change from SET data or vertical accretion from MH data. The resulting data frame has the mean station-level cumulative change for each sampling date.
...
The function plot_station_cumu can be used to plot station-level cumulative surface elevation change from SET data or vertical accretion from MH data.
...
The function calc_linear_rates() can be used to calculate simple linear rates of change at the station-level in mm/yr.
...
The functions plot_set_rate_comps() and plot_set_mh_comps() can be used to create 'tie fighter' plots that are helpful for visually comparing rates between stations:
