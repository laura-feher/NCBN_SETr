...
#'
#' @param data a data frame with one row per pin reading, and the following columns, named exactly: network_code, park_code, station_code, SET_direction, pin_height_mm
...
plot_hist_arm <- function(data, columns = 4, scales = "free_y"){
    ...
    plot_data <- data %>%
        mutate(set_id = paste(network_code, park_code, station_code, sep = "_"))
